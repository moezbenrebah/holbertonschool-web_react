...
const FocusScope = React.forwardRef<FocusScopeElement, FocusScopeProps>(
  (props, forwardedRef) => {
    ...
    const isFocusHandlingRef = React.useRef(false);
    ...
    React.useEffect(() => {
      if (trapped) {
        function handleFocusIn(event: FocusEvent) {
          if (focusScope.paused || !container || isFocusHandlingRef.current) {
            return;
          }
          ...
          if (container.contains(target)) {
            ...
            isFocusHandlingRef.current = true;
            focus(lastFocusedElementRef.current, { select: true });
            // Réinitialiser le drapeau après un court délai
            setTimeout(() => {
              isFocusHandlingRef.current = false;
            }, 0);
          }
        }
        ...
        function handleFocusOut(event: FocusEvent) {
          ...
          if (!container.contains(relatedTarget) && lastFocusedElementRef.current) {
            isFocusHandlingRef.current = true;
            focus(lastFocusedElementRef.current, { select: true });
            // Réinitialiser le drapeau après un court délai
            setTimeout(() => {
